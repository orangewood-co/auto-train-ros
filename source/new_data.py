...
class NewData:
    ...
    def _get_frame(self, timeout=30.0):
        ...
        if not self._frame_event.wait(timeout=timeout):
            raise TimeoutError(f"No image received on '{self.image_topic}' within {timeout}s")
        with self._frame_lock:
    ...
    def capture_pred(self, box_threshold, text_threshold):
        ...
        Args:
            ...
            - text_threshold (float): Text threshold for Grounding DINO
        """
        sub = self.node.create_subscription(ROSImage, self.image_topic, self._image_callback, 10)
        try:
            self.logger.info(f"Waiting for images on '{self.image_topic}' (30s timeout)...")
    ...
    def split_and_yaml(self):
        ...
        yaml_content = {"path": split_path, "train": f"{upper_folder}/train", "val": f"{upper_folder}/val", "names": {index: value for index, value in enumerate(candidate_labels)}}
